...
old_code = """                val annotatedOutput = remember<androidx.compose.ui.text.AnnotatedString>(rawText, cursorVisible, isDark) {
...
new_code = """                val annotatedOutput = remember<androidx.compose.ui.text.AnnotatedString>(rawText, cursorVisible, isDark) {
                    try {
                        val defaultTextColor: Color = if (isDark) Color(0xFF4ADE80) else Color(0xFF38BDF8)
                        val parsed = AnsiTextParser.parse(rawText + cursorChar, defaultColor = defaultTextColor)
                        AnsiTextParser.applyBasicSyntaxHighlighting(parsed, isDark)
